import tkinter
from quiz_brain import QuizBrain

THEME_COLOR = "#375362"
QUESTION_FONT = ("Arial", 20, "italic")
SCORE_FONT = ("Arial", 10, "bold")


class QuizInterface:
    def __init__(self, quiz_brain: QuizBrain):
        self.quiz = quiz_brain

        self.window = tkinter.Tk()
        self.window.title("Quizzler")
        self.window.config(bg=THEME_COLOR, padx=20, pady=20)

        # create canvas
        self.canvas = tkinter.Canvas(width=300, height=250,
                                     bg="white")
        self.canvas.grid(row=1, column=0,
                         columnspan=2,
                         pady=50)

        # create question text
        self.question_text = self.canvas.create_text(
            150, 125,
            width=280,
            text="Some question text",
            fill=THEME_COLOR,
            font=QUESTION_FONT,
        )

        # create label
        self.score_label = tkinter.Label(text="Score: 0",
                                         fg="white",
                                         bg=THEME_COLOR,
                                         font=SCORE_FONT)
        self.score_label.grid(row=0, column=1)

        # load images
        true_image = tkinter.PhotoImage(file="images/true.png")
        false_image = tkinter.PhotoImage(file="images/false.png")

        # create buttons
        self.true_button = tkinter.Button(
            image=true_image,
            highlightthickness=0,
            command=self.answer_in_true,
        )
        self.true_button.grid(row=2, column=0)

        self.false_button = tkinter.Button(
            image=false_image,
            highlightthickness=0,
            command=self.answer_in_false,
        )
        self.false_button.grid(row=2, column=1)

        self.get_next_question()

        self.window.mainloop()

    def get_next_question(self):
        if self.quiz.still_has_questions():
            q_text = self.quiz.next_question()
            self.canvas.itemconfig(self.question_text,
                                   text=q_text)
            self.score_label.config(text=f"Score: {self.quiz.score}")

        else:
            self.canvas.itemconfig(self.question_text, text="You've reached the end of the quiz.")
            self.true_button.config(state="disabled")
            self.false_button.config(state="disabled")

    def answer_in_true(self):
        self.give_feedback(self.quiz.check_answer("True"))

    def answer_in_false(self):
        self.give_feedback(self.quiz.check_answer("False"))

    def change_background(self, is_right):
        if is_right:
            self.canvas.config(bg="green")
        else:
            self.canvas.config(bg="red")

    def default_background(self):
        self.canvas.config(bg="white")

    def give_feedback(self, is_right):
        self.change_background(is_right)
        self.window.after(1000, func=self.default_background)
        self.get_next_question()
